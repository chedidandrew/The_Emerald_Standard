...
public final class BankerProfessionNeoForge {
    private static final DeferredRegister<Block> BLOCKS = DeferredRegister.create(Registries.BLOCK, BankerProfessionSupport.MOD_ID);
    private static final DeferredRegister<Item> ITEMS = DeferredRegister.create(Registries.ITEM, BankerProfessionSupport.MOD_ID);
    private static final DeferredRegister<PoiType> POI_TYPES = DeferredRegister.create(Registries.POINT_OF_INTEREST_TYPE, BankerProfessionSupport.MOD_ID);
    private static final DeferredRegister<VillagerProfession> PROFESSIONS = DeferredRegister.create(Registries.VILLAGER_PROFESSION, BankerProfessionSupport.MOD_ID);
    ...
    public static final DeferredHolder<Item, Item> EXCHANGE_DESK_ITEM = ITEMS.register(
            "exchange_desk", () -> new BlockItem(EXCHANGE_DESK.get(), BankerProfessionSupport.createExchangeDeskItemProperties()));
    public static final DeferredHolder<Item, Item> HANDBOOK = ITEMS.register("handbook", HandbookReaderItem::new);
    ...
    private static void addCreativeTabContents(BuildCreativeModeTabContentsEvent event) {
        if (event.getTabKey().equals(CreativeModeTabs.FUNCTIONAL_BLOCKS)) event.accept(EXCHANGE_DESK_ITEM.get());
        if (event.getTabKey().equals(CreativeModeTabs.TOOLS_AND_UTILITIES)) event.accept(HANDBOOK.get());
    }
}
