...
The six dashboard pages are Overview, Market, Banking, Exchange, Village, and Fund. The menu synchronizes bounded account, portfolio-analytics, market, commodity-history, donor, and local-village snapshots rather than exposing the complete world-account map or private economy seed. Irreversible or risky actions use a time-limited server-owned two-step confirmation, and the additive Fund draft is also maintained and bounded by the server.
