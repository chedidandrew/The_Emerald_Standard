...
- Advanced persistence from format 9 to format 10 for village architectural character and biome dialect plus each project's architecture schema, recipe, road-facing rotation, frozen visual stage, and independent road cursor state, then to format 11 for explicit Banker-only fallback provenance and format 12 for durable authored Bank structure versions. Format-9 and earlier projects remain `legacy_v1` without reroll, repositioning, or modular conversion; projects approved under format 10 or later use `modular_v1`.
