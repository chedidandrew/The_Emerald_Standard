...
final class VillageConstructionActivity {
    ...
    record Site(String tag, UUID village, long project, BlockPos origin, BlockPos corner) { }
    private VillageConstructionActivity() { }
    static void reset() { CARGO.clear(); }
        ...
        if (enabled) {
            ...
            for (var snapshot : economy.villageSnapshots()) {
                ...
                for (var project : village.projects) {
                    if (project.originPos == 0 || project.materializedComplete) continue;
    ...
    static void update(ServerLevel level, List<Site> sites) {
        ...
        for (Entity entity : entities) {
            ...
            if (entity.entityTags().stream().noneMatch(t -> t.startsWith(JOB))) entity.removeTag(DELIVERY);
        ...
        for (Site site : sites) {
            ...
            for (Villager worker : assigned) {
                if (workers.size() < 2 && !worker.isBaby() && !BankerAccess.isBanker(worker)
                        && worker.blockPosition().distSqr(site.origin) < 96 * 96) workers.add(worker);
                else worker.removeTag(site.tag);
            }
            for (Villager candidate : villagers.stream().filter(v -> available(v)
            ...
            for (int i = 0; i < workers.size(); i++) {
                ...
                if (walkable(level, target)) worker.getBrain().setMemory(
                        net.minecraft.world.entity.ai.memory.MemoryModuleType.WALK_TARGET,
                        new net.minecraft.world.entity.ai.memory.WalkTarget(target, .55f, 0));
                ...
                if (worker.entityTags().contains(DELIVERY) && entities.stream().noneMatch(e -> e.isAlive()
                    ...
                    positionCargo(load, worker); level.addFreshEntity(load); entities.add(load);
                }
            }
        }
