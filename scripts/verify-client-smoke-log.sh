#!/usr/bin/env bash
set -euo pipefail
if [[ $# -lt 3 || $# -gt 4 || ( "$1" != fabric && "$1" != neoforge ) ]]; then
    echo "Usage: $0 <fabric|neoforge> <log> <exit-status> [expected-processes]" >&2
    exit 2
fi
LOADER="$1"
LOG_FILE="$2"
...
if [[ $(grep -Ec 'Emerald Handbook page layout verified for [1-9][0-9]* pages' "$LOG_FILE") -ne "$expected" ]]; then
...
for marker in \
    ...
    if [[ $(grep -Fc "$marker" "$LOG_FILE") -ne "$expected" ]]; then
...
if [[ "${TES_TEST_MODMENU:-absent}" == present ]] \
    && [[ $(grep -Fc "The Emerald Standard Mod Menu configuration factory verified" "$LOG_FILE") -ne "$expected" ]]; then
    echo "Mod Menu configuration was not verified in both processes" >&2
    exit 1
fi
if [[ "${TES_TEST_MODMENU:-absent}" == absent ]] \
    && [[ $(grep -Fc "The Emerald Standard optional Mod Menu absence verified" "$LOG_FILE") -ne "$expected" ]]; then
    echo "Optional Mod Menu absence was not verified in both processes" >&2
    exit 1
fi
if [[ "$LOADER" == neoforge ]] \
    && [[ $(grep -Fc "The Emerald Standard NeoForge configuration factory verified" "$LOG_FILE") -ne "$expected" ]]; then
