...
- Save format 11 uses a required magic identifier, mandatory core fields, and SHA-256 checksums.
...
- Village identities, residents, incidents, full pre-player-damage village counterfactuals, projects, exact project bounds, village architectural character and biome dialect, project architecture version and recipe, rotation, frozen visual stage, stable road anchor, independent building/road progress, construction retry state, bank anchors, explicit Banker-only fallback provenance, and lifecycle state are persistent.
- A configurable transaction cooldown protects servers from repeated button or packet spam.

The 0.4 line first upgraded beta.4 format-7 worlds to format 8 for the expanded project catalog. Beta.2 advanced those worlds to format 9 for portfolio analytics, multiple term positions, commodity and personal history, Prosperity Funds, and donor records. Format 10 added modular village architecture, and the current format 11 adds explicit Banker-only fallback provenance. Every project read from format 9 or earlier remains `legacy_v1` with its existing origin, order, bounds, and progress; it is never rerolled or converted into a modular building. Projects approved under format 10 or later use `modular_v1`. Formats 10 and earlier never infer retryable fallback status from an ordinary Bank anchor, preventing ambiguous legacy or crash-interrupted markers from creating a duplicate structure. Older holdings still receive an explicitly inferred opening cost basis because their historical executions were not recorded. Older builds reject format 11 instead of silently discarding its fields or falling back to stale data. Keep a pre-upgrade world backup if you may need to downgrade.
