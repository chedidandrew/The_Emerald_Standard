...
Each village persists one of four architectural characters and a biome-derived material dialect so its projects read as one settlement. Each newly approved project then combines one of three silhouettes, three roof families, three frontage families, and a standard or mirrored layout: **54 deterministic recipes per project type**. Silhouettes are selected from the least-used family for that project type, and an anti-repeat rule prevents the newest project from exactly matching its immediate predecessor. The curated grammar supplies type-specific interiors, work areas, and later landmarks instead of scattering individual blocks at random.

After a safe lot and stable road anchor are chosen, the building rotates to face that connection. Its recipe, rotation, shallow foundation supports, and current visual stage remain stable across reloads. Town- and city-stage additions are append-only and preflighted before their persisted target stage changes, so a later tier decline never removes earlier work. The dirt-path/gravel road has its own persisted cursor and completion state: it advances only through loaded chunks after the building is complete, adopts existing TES-style path cells, and skips unsafe, protected, occupied, or non-terrain cells without changing the building's bounds, completion, integrity, or economic authority.
...
- Economic completion activates a project's housing and production effects even while its physical template is still queued. A low-frequency integrity audit can suspend the benefits of a structure that had materialized and is later found missing an authored block; the player must restore that block before the audit reactivates it, so furnishings cannot become renewable drops. Safe append-only design-stage upgrades still use the guarded construction queue.
...
- Save format 12 uses a required magic identifier, mandatory core fields, and SHA-256 checksums.
...
- Village identities, residents, incidents, full pre-player-damage village counterfactuals, projects, exact project bounds, village architectural character and biome dialect, project architecture version and recipe, rotation, frozen visual stage, stable road anchor, independent building/road progress, construction retry state, bank anchors, explicit Banker-only fallback provenance, authored Bank structure versions, and lifecycle state are persistent.
- A configurable transaction cooldown protects servers from repeated button or packet spam.

The 0.4 line first upgraded beta.4 format-7 worlds to format 8 for the expanded project catalog. Beta.2 advanced those worlds to format 9 for portfolio analytics, multiple term positions, commodity and personal history, Prosperity Funds, and donor records. Format 10 added modular village architecture, format 11 added explicit Banker-only fallback provenance, and the current format 12 adds authored structure versions for anchored, non-fallback generated Banks. Every project read from format 9 or earlier remains `legacy_v1` with its existing origin, order, bounds, and progress; it is never rerolled or converted into a modular building. Projects approved under format 10 or later use `modular_v1`. Formats 10 and earlier never infer retryable fallback status from an ordinary Bank anchor, preventing ambiguous legacy or crash-interrupted markers from creating a duplicate structure. Banks read from format 11 or earlier deliberately begin without a trusted structure version; only a successful signature-guarded in-world upgrade records version 2. Older holdings still receive an explicitly inferred opening cost basis because their historical executions were not recorded. Older builds reject format 12 instead of silently discarding its fields or falling back to stale data. Keep a pre-upgrade world backup if you may need to downgrade.

Materialized projects are audited gradually. A missing authored block removes that project's economic authority but is not regenerated; replacing the authored block lets a later audit restore the benefits without creating an item farm. Append-only template upgrades remain guarded and never overwrite solid player blocks, block entities, protected placements, or unloaded chunks. Bank markers and Minecraft chunk saves are still not cross-file atomic, so a crash between those writes can leave a non-provenanced marker without a structure. Fallback Banker access remains available, but that ambiguous Bank is not automatically rebuilt. Player financial data remains world-level and intact.
