...
- Save format 10 uses a required magic identifier, mandatory core fields, and SHA-256 checksums.
...
- Village identities, residents, incidents, full pre-player-damage village counterfactuals, projects, exact project bounds, village architectural character and biome dialect, project architecture version and recipe, rotation, frozen visual stage, stable road anchor, independent building/road progress, construction retry state, bank anchors, and lifecycle state are persistent.
- A configurable transaction cooldown protects servers from repeated button or packet spam.

The 0.4 line first upgraded beta.4 format-7 worlds to format 8 for the expanded project catalog. Beta.2 advanced those worlds to format 9 for portfolio analytics, multiple term positions, commodity and personal history, Prosperity Funds, and donor records. The current line advances supported format-9 and earlier worlds to format 10 for modular village architecture. Every project read from format 9 or earlier remains `legacy_v1` with its existing origin, order, bounds, and progress; it is never rerolled or converted into a modular building. Only projects approved under format 10 use `modular_v1`. Older holdings still receive an explicitly inferred opening cost basis because their historical executions were not recorded. Older builds deliberately reject format 10 instead of silently discarding its fields or falling back to stale data. Keep a pre-upgrade world backup if you may need to downgrade.
