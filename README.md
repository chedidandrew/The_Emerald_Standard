...
> Current source status: **unreleased `0.4.0-beta.5` development candidate**, not a stable release. The latest published download remains the older [`0.4.0-beta.2` prerelease](https://github.com/chedidandrew/The_Emerald_Standard/releases/tag/v0.4.0-beta.2); it does not include the later architecture, handbook reader, in-game settings, or Peaceful growth and structure loot work. CI artifacts are experimental exact-commit builds, not published releases. The [beta.4 verification record](docs/reviews/2026-09-09-beta4-final-validation.md) is historical, not verification of beta.5; see [Peaceful growth and loot](docs/PEACEFUL_GROWTH_AND_LOOT.md) for the new behavior. The broader human gameplay matrix is not certified. Normal gameplay centers on Banker villagers, Village Banks, the graphical dashboard and optional Village Prosperity. Commands are for administrators and diagnostics.
...
Cities can add districts without a fixed city-size cap. Open **Banker > Village > City expansion**
...
and each active site places up to two blocks per second independently. See [construction and entrances](docs/PROGRESSIVE_CONSTRUCTION.md).
...
2. To test this source candidate, build or obtain exactly one `0.4.0-beta.5` playable JAR from the same exact commit, then install that same Fabric or NeoForge JAR on the server and every connecting client. Never install both loader JARs, do not use a `-sources.jar`, and do not mix builds from different commits. The linked public `0.4.0-beta.2` artifacts are older and do not include the Unreleased features described by this README.
...
**Peaceful difficulty automatically boosts village growth:** more production, lower food consumption, faster development and safety/prosperity recovery, and a 20x eligible settler growth chance. Easy, Normal and Hard share the normal difficulty profile. Housing, food, safety, construction protection and loaded-chunk requirements still apply. Each district retains tier 5, 64 simulated residents including queued settlers, and 12 prosperity projects with at most six housing projects; cities can add districts without a fixed district-count cap, with increasing upkeep naturally limiting expansion.
...
- Construction retains up to two tagged builder assignments per loaded site, with short delivery trips and supported, display-only scaffolding/material props. There is no custom arm animation or physical inventory logistics. Cleanup touches only owned entities, never player blocks. New lots can include optional roadside squares, gardens and seating nooks. Upkeep advice stays in the existing Village details; construction status is logged, not exposed as a management screen. See [background village life](docs/BACKGROUND_VILLAGE_LIFE.md).
